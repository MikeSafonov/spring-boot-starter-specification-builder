...
public class GreaterThanPredicateBuilder extends SimplePredicateBuilder {
    ...
    public GreaterThanPredicateBuilder(ExpressionBuilder expressionBuilder, FieldWithValue field) {
        super(expressionBuilder, field);
    }

    @Override
    public Predicate build(Root<?> root, CriteriaQuery<?> q, CriteriaBuilder cb) {
        return cb.greaterThan(getExpression(root), (Comparable) field.getValue());
    }
}
